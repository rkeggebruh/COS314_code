...
public class GAWithLS extends Reader {
    ...
    public GAWithLS(Knapsack initalKnapsack) {
    ...
    public void applyLocalSearch() {
        LocalSearch localSearch = new LocalSearch(knapsack);
        for (int i = 0; i < knapsackPop.size(); i++) {
            Boolean[] improvedSolution = localSearch.runLocalSearch(knapsackPop.get(i));
            knapsackPop.set(i, improvedSolution);
        }
    }
